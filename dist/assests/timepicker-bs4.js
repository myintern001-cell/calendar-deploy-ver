/**
 * Time picker for Bootstrap 5 (Modified from Bootstrap 4 version)
 * Modified to use Bootstrap Icons instead of Font Awesome
...
function updateView($input)
{
	...
	$content.find('.timepicker-btns button').removeClass('fw-bold').filter('[data-unit="' + view + '"]').addClass('fw-bold').text(text);
...
function updatePicker($input)
{
	...
	let html = '<div class="clock-input' + (clock_enabled ? '' : ' d-none') + '">'
		...
		+ '<div class="btn-group">'
		+ '<button type="button" class="btn px-2 fw-bold hour-btn" data-unit="hour">' + viewTime.format('hh') + '</button>'
		...
		+ '<td class="text-center align-bottom" colspan="3"><a id="' + input_id + '-picker-add-link" class="btn btn-link px-1 mx-0 chevron-btn" data-step="1" href="javascript:void(0)"><i class="bi bi-chevron-up fs-5"></i></a></td>'
		...
		+ '<td class="text-center" colspan="3"><a id="' + input_id + '-picker-sub-link" class="btn btn-link px-1 mx-0 chevron-btn" data-step="-1" href="javascript:void(0)"><i class="bi bi-chevron-down fs-5"></i></a></td>'
		...
		+ '</tr></tbody></table>').replace(/{{(\w+)}}/g, function (match, position) {
			if (position == 0)
			{
				return '<button type="button" id="'+ input_id + '-picker-center-btn" class="btn btn-outline-dark border-white fw-bold pos-0" data-hour="' + viewHour + '"'
		...
		+ '<td><a class="btn btn-link px-1 mx-0 chevron-btn" data-unit="hour" data-step="1" href="javascript:void(0)"><i class="bi bi-chevron-up fs-5"></i></a></td>'
		+ '<td></td>'
		+ '<td><a class="btn btn-link px-1 mx-0 chevron-btn" data-unit="minute" data-step="1" href="javascript:void(0)"><i class="bi bi-chevron-up fs-5"></i></a></td>'
		+ (has_second ? '<td></td><td><a class="btn btn-link px-1 mx-0 chevron-btn" data-unit="second" data-step="1" href="javascript:void(0)"><i class="bi bi-chevron-up fs-5"></i></a></td>' : '')
		+ (clock_24 ? '' : '<td><a class="btn btn-link px-1 mx-0 chevron-btn" data-unit="meridiem" data-step="1" href="javascript:void(0)"><i class="bi bi-chevron-up fs-5"></i></a></td>')
		+ '</tr><tr>'
		...
		+ '</tr><tr>'
		+ '<td><a class="btn btn-link px-1 mx-0 chevron-btn" data-unit="hour" data-step="-1" href="javascript:void(0)"><i class="bi bi-chevron-down fs-5"></i></a></td>'
		+ '<td></td>'
		+ '<td><a class="btn btn-link px-1 mx-0 chevron-btn" data-unit="minute" data-step="-1" href="javascript:void(0)"><i class="bi bi-chevron-down fs-5"></i></a></td>'
		+ (has_second ? '<td></td><td><a class="btn btn-link px-1 mx-0 chevron-btn" data-unit="second" data-step="-1" href="javascript:void(0)"><i class="bi bi-chevron-down fs-5"></i></a></td>' : '')
		+ (clock_24 ? '' : '<td><a class="btn btn-link px-1 mx-0 chevron-btn" data-unit="meridiem" data-step="-1" href="javascript:void(0)"><i class="bi bi-chevron-down fs-5"></i></a></td>')
		+ '</tr></table></div>'
		+ '<div class="d-flex justify-content-between">'
		+ '<div class="invisible"><button type="button" class="btn btn-link input-toggle-btn' + (clock_enabled ? ' d-none' : '') + '" data-input="clock"><i class="bi bi-clock"></i></button><button type="button" class="btn btn-link input-toggle-btn' + (clock_enabled ? '' : ' d-none') + '" data-input="keyboard"><i class="bi bi-keyboard"></i></button></div>'
		+ '<div><button type="button" class="btn btn-secondary mx-1 cancel-btn" data-bs-dismiss="popover" title="Cancel"><i class="bi bi-x"></i></button><button type="button" class="btn btn-primary mx-1 submit-btn" title="OK"><i class="bi bi-check"></i></button></div>'
		+ '</div>';
	...
	$content.find('.submit-btn').on('click', function () {
		const popoverInstance = bootstrap.Popover.getInstance($input[0]);
		$input.val($input.data('viewtime').format($input.data('options').format)).data('view', null).trigger('change');
		if (popoverInstance) {
			popoverInstance.hide();
		}
	});
	...
	$table.find('button').on('click', function () {
		...
		{
			const popoverInstance = bootstrap.Popover.getInstance($input[0]);
			$input.val(viewTime.format(options.format)).data('view', null).trigger('change');
			if (popoverInstance) {
				popoverInstance.hide();
			}
		}
...
jQuery.fn.timepicker = function (options) {
	if (typeof options == 'string')
	{
		...
		{
			case 'show':
				const popoverInstance = bootstrap.Popover.getInstance(this[0]);
				if (popoverInstance) {
					popoverInstance.show();
				}
				break;
			case 'hide':
				const popoverHide = bootstrap.Popover.getInstance(this[0]);
				if (popoverHide) {
					popoverHide.hide();
				}
				break;
	...
	return this.each(function () {
		...
		const popover = new bootstrap.Popover(this, {
			...
			placement: placement,
			sanitize: false,
			container: '#modal-view-event-add', //=========================config ให้เข้ากับ js =============================
			title: '<button class="btn-close mt-n1" data-bs-dismiss="popover"></button>' + (($label.length > 0) ? $label.html() : 'Time'),
			template: '<div id="' + input_id + '-picker-popover" class="popover timepicker-popover bs-popover-' + placement + '" role="tooltip"><div class="popover-arrow"></div><h3 class="popover-header"></h3><div id="' + input_id + '-popover-body" class="popover-body border-bottom"></div><div class="popover-footer bg-light text-end px-3 py-2 rounded-lg" hidden="hidden"><button type="button" class="btn btn-secondary btn-sm" title="Close the picker" data-bs-dismiss="popover"><i class="fas fa-times"></i> Close</button></div></div>',
		...
		this.addEventListener('shown.bs.popover', function () {
			...
			{
				//jQuery('#' + input_id + '-picker-content').find('.hour-input').select();  ปิดไว้ยังไม่ใช้
			}
		});

		this.addEventListener('inserted.bs.popover', function () {
			jQuery('.popover').find('[data-bs-dismiss="popover"]').on('click', function () {
				const popoverInstance = bootstrap.Popover.getInstance($input[0]);
				if (popoverInstance) {
					popoverInstance.hide();
				}
		...
		this.addEventListener('hide.bs.popover', function () {
